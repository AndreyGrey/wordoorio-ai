...
**Claude Code может автоматизировать:**
- ✅ Установка зависимостей (pip install)
- ✅ Создание .env файла
- ✅ Установка Yandex Cloud CLI
- ✅ Создание Service Account keys
- ✅ Генерация base64 для GitHub Secrets
- ✅ Проверка статуса ресурсов
- ✅ Деплой через git push
- ✅ Диагностика проблем (логи, статусы)

**Требуется ручное действие пользователя:**
- ⚠️ **Авторизация в Yandex Cloud** (`yc init` требует OAuth через браузер)
- ⚠️ **Добавление GitHub Secrets** (нужен веб-интерфейс GitHub)

**Как использовать:**
1. Скажи Claude Code: "Настрой деплой Wordoorio согласно SERVERLESS_DEPLOYMENT.md"
2. Claude Code выполнит все автоматические шаги
3. Когда потребуется ручное действие - он попросит тебя выполнить и продолжит
...
**YDB (Yandex Database) - Serverless NoSQL/SQL база данных:**
- Полностью управляемый сервис Yandex Cloud
- Данные персистентны между деплоями контейнера
- Автоматическое масштабирование и репликация
- Оплата только за фактическое использование (pay-per-request)
- Авторизация через MetadataUrlCredentials (сервисный аккаунт)

**Особенности:**
- ✅ Данные НЕ теряются при деплое (в отличие от SQLite в контейнере)
- ✅ Автоматические бэкапы (встроенная функция YDB)
- ✅ Масштабируемость (автоматически обрабатывает любую нагрузку)
- ✅ Низкая стоимость (~50-150₽/месяц для небольших проектов)

**Настройка:**

1. Создать serverless YDB базу через Yandex Cloud Console
2. Настроить права доступа для сервисного аккаунта:
   ```bash
   yc resource-manager folder add-access-binding <FOLDER_ID> \
     --service-account-id <SA_ID> \
     --role ydb.editor
   ```
3. Создать таблицы через скрипт:
   ```bash
   python create_ydb_schema.py
   ```
4. Добавить переменные окружения в контейнер:
   - `YDB_ENDPOINT` - endpoint YDB базы
   - `YDB_DATABASE` - путь к базе данных

**Документация:** См. [DATABASE_STORAGE.md](../DATABASE_STORAGE.md) для деталей миграции SQLite → YDB
...
**Обновление v1.1:** Добавлена трехфазная стратегия управления базой данных (SQLite → Backup → PostgreSQL)
