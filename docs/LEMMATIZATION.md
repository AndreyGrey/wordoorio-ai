...
**Ключевой принцип:** `lemmatize_russian()` ВСЕГДА вызывается для RU перевода.

**Почему не делаем условную обработку:**

Попытка "если EN причастие → RU не трогать" неверна:
- AI вернул "засыпанная" (ж.р.) → останется "засыпанная"
- А должно быть "засыпанный" (м.р. ед.ч.)

**`lemmatize_russian()` уже корректно обрабатывает причастия:**
- "засыпанная" (PRTF) → `inflect()` → "засыпанный" ✓
- "нарушающая" (PRTF) → `inflect()` → "нарушающий" ✓

**Проблема возникает, когда AI возвращает не ту часть речи:**
- EN: "peppered" (причастие)
- AI вернул RU: "засыпали" (VERB, глагол — неправильно!)
- `lemmatize_russian()` → "засыпать" (инфинитив)
- Ожидалось: "засыпанный" (причастие)

**Это ошибка промпта AI-агента, а не кода.** Промпт должен требовать:
> "Если EN = причастие, RU перевод тоже должен быть причастием"

**Текущая логика:**
```python
# Всегда вызываем lemmatize_russian()
main_translation = lemmatize_russian(raw_translation).lower()
```

`lemmatize_with_pos()` используется только для определения, является ли EN слово причастием (для логирования и возможной будущей логики).
...
| 9183320 | Добавлен `lemmatize_with_pos()`, попытка синхронизации EN/RU |
| 63da24b | Откат: всегда вызываем `lemmatize_russian()` |

---

## 9. Известные ограничения (нерешенные)

### 9.1 AI возвращает глагол вместо причастия

**Проблема:**
- EN: "peppered" (причастие)
- AI вернул: "засыпали" (глагол) — должен был вернуть "засыпанный"
- `lemmatize_russian()` → "засыпать" (инфинитив)

**Решение:** улучшить промпт AI-агента, добавить правило:
> "Если английское слово — причастие (Participle), русский перевод тоже должен быть причастием"

### 9.2 `lemmatize_with_pos()` не используется для логики

Функция `lemmatize_with_pos()` добавлена, но `is_participle` не влияет на обработку RU.
Может использоваться в будущем для:
- Логирования несоответствий EN/RU POS
- Валидации ответов AI-агента
