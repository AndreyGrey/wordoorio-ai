...
# Test accounts (synced with telegram_bot.py)
TELEGRAM_TEST_ACCOUNTS = {
    'andrew': {'password': 'test123', 'user_id': 1},
    'friend1': {'password': 'test123', 'user_id': 2},
    'friend2': {'password': 'test123', 'user_id': 3},
...
def get_tg_user_id(telegram_id: int):
...
async def tg_start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user:
        ...
        await update.message.reply_text(
            ...
            reply_markup=get_tg_main_keyboard()
        )
    else:
        await update.message.reply_text(
            "Привет! 👋\n\n"
            "Для начала привяжи Telegram к аккаунту:\n"
            "`/login username password`\n\n"
            "Например:\n"
            "`/login andrew test123`",
            parse_mode='Markdown'
...
async def tg_login_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if username not in TELEGRAM_TEST_ACCOUNTS or TELEGRAM_TEST_ACCOUNTS[username]['password'] != password:
    ...
    if success:
        await update.message.reply_text(
            f"✅ Telegram привязан к `{username}`!",
            reply_markup=get_tg_main_keyboard(),
            parse_mode='Markdown'
        )
    else:
...
async def tg_start_training(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Start training session"""
    telegram_id = update.effective_user.id
    user_id = get_tg_user_id(telegram_id)
    ...
    try:
        ...
        if not words:
            await loading_msg.edit_text("📚 В словаре пока нет слов.\nДобавь через веб-интерфейс!")
            return ConversationHandler.END
        ...
        if not test_ids:
            await loading_msg.edit_text("⚠️ Не удалось создать тесты.")
            return ConversationHandler.END
        ...
        return await tg_show_test(update, context)

    except Exception as e:
        logger.error(f"Training error: {e}")
        await loading_msg.edit_text(f"⚠️ Ошибка: {str(e)[:100]}")
        return ConversationHandler.END


async def tg_show_test(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show current test"""
    tests = context.user_data.get('tests', [])
    index = context.user_data.get('current_index', 0)

    if index >= len(tests):
        return await tg_show_results(update, context)
    ...
    for i, option in enumerate(test['options']):
        keyboard.append([InlineKeyboardButton(option['text'], callback_data=f"ans:{i}")])
...
async def tg_handle_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle answer"""
    ...
    if is_correct:
        ...
        status = "🎓 Выучено!" if new_rating >= 10 else f"Рейтинг: {new_rating}/10"
        text = f"✅ *Верно!*\n\n{test['word']} → {correct_option['text']}\n\n{status} {stars}"
    else:
        text = f"❌ *Неверно*\n\n{test['word']} → *{correct_option['text']}*\n\nРейтинг сброшен: 0/10"
...
async def tg_next_test(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Next test"""
    query = update.callback_query
    await query.answer()
...
async def tg_show_results(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show results"""
    ...
    if accuracy >= 80:
        ...
        emoji, comment = "💪", "Продолжай практиковаться!"
...
async def tg_restart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Restart training"""
    query = update.callback_query
    await query.answer()
    await query.message.delete()

    # Create fake message for start_training
    class FakeMsg:
    ...
    fake_update = FakeUpdate(FakeMsg(query.message.chat), query.from_user)
    return await tg_start_training(fake_update, context)


async def tg_statistics(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show statistics"""
    telegram_id = update.effective_user.id
    user_id = get_tg_user_id(telegram_id)

    if not user_id:
        await update.message.reply_text("❌ Сначала авторизуйтесь: `/login`", parse_mode='Markdown')
        return

    stats = db.get_dictionary_stats(user_id)
    total = stats.get('total', 0)
    progress = round((stats.get('learned', 0) / total) * 100) if total > 0 else 0

    text = (
        f"📊 *Статистика*\n\n"
        f"📚 Всего: {total}\n"
        f"🆕 Новых: {stats.get('new', 0)}\n"
        f"📖 Изучаю: {stats.get('learning', 0)}\n"
        f"✅ Выучено: {stats.get('learned', 0)}\n\n"
        f"🎯 Прогресс: {progress}%"
    )
    await update.message.reply_text(text, reply_markup=get_tg_main_keyboard(), parse_mode='Markdown')


async def tg_dictionary(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show dictionary"""
    telegram_id = update.effective_user.id
    user_id = get_tg_user_id(telegram_id)
    ...
    if not words:
        await update.message.reply_text("📚 Словарь пуст.\nДобавляй слова через веб!", reply_markup=get_tg_main_keyboard())
        return

    text = "📚 *Словарь* (10 последних)\n\n"
    for w in words:
        icon = "✅" if w.get('status') == 'learned' else "📖" if w.get('status') == 'learning' else "🆕"
        translation = db.get_translation_for_word(w.get('id')) or "—"
        text += f"{icon} *{w.get('lemma')}* — {translation}\n"

    await update.message.reply_text(text, reply_markup=get_tg_main_keyboard(), parse_mode='Markdown')


async def tg_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle text messages"""
    text = update.message.text
    if text == "💪 Начать тренировку":
        return await tg_start_training(update, context)
    elif text == "📊 Моя статистика":
        return await tg_statistics(update, context)
    elif text == "📚 Мой словарь":
        return await tg_dictionary(update, context)
    else:
        await update.message.reply_text("Используй кнопки ниже:", reply_markup=get_tg_main_keyboard())
...
def setup_telegram_handlers(app: Application):
    ...
    training_handler = ConversationHandler(
        entry_points=[
            CommandHandler("train", tg_start_training),
            MessageHandler(filters.Regex("^💪 Начать тренировку$"), tg_start_training),
        ],
        states={
            TRAINING: [
                CallbackQueryHandler(tg_handle_answer, pattern=r"^ans:\d+$"),
                CallbackQueryHandler(tg_next_test, pattern=r"^next$"),
                CallbackQueryHandler(tg_restart, pattern=r"^restart$"),
            ],
        },
        fallbacks=[CallbackQueryHandler(tg_restart, pattern=r"^restart$")],
        allow_reentry=True,
    )

    app.add_handler(CommandHandler("start", tg_start_command))
    app.add_handler(CommandHandler("login", tg_login_command))
    app.add_handler(training_handler)
    app.add_handler(MessageHandler(filters.Regex("^📊 Моя статистика$"), tg_statistics))
    app.add_handler(MessageHandler(filters.Regex("^📚 Мой словарь$"), tg_dictionary))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, tg_text_handler))
...
def set_telegram_webhook():
    ...
    try:
        ...
        return jsonify({'success': result, 'url': webhook_url})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
    finally:
        loop.close()
